...
public class Menu extends javax.swing.JFrame implements Runnable, ThreadFactory {
    ...
    private static int numCam = -1;

    
    private static int getnumCam() {
        numCam++;
        return numCam;
    }
    ...
    boolean consulta(String cod) throws SQLException {
        st = cConexion.createStatement();
        String sql = "select no_control, nombre from a_d_p where no_control = '" + cod + "'";
        ResultSet resultado = st.executeQuery(sql);
        String res = null;
        while (resultado.next()) {
            res = resultado.getString("no_control");
            nombre = resultado.getString("nombre");
        }
        if ((res == null ? cod == null : res.equals(cod)) && res != null) {
            return true;
        }
    ...
    private void initComponents() {
        ...
        result_field.setBackground(new java.awt.Color(153, 255, 153));
    ...
    private void initWebcam() {
        ...
        webcam = Webcam.getWebcams().get(Menu.getnumCam()); //0 is default webcam
    ...
    public void run() {
        do {
            ...
            if (result != null) {
                ...
                try {
                    if (consulta(result.getText())) {
                        result_field.setText("BIENVENIDO " + nombre + " PUEDES PASAR");
                    } else {
                        result_field.setText("CODIGO INVALIDO");
                    }
